...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 4, 384, 768).to(device)

    # Test with default settings
    model = ViT(image_size=x.shape[2:], patch_size=6,
        in_chans=x.shape[1], out_chans=1, dim=64,
        depth=4, heads=8, mlp_dim=128, dim_head=64).to(device)  # 放到 GPU
    ...
    model.train()
    y = model(x)
    print(f"Training mode output shape: {y.shape}")
    # Forward pass in evaluation mode
    model.eval()
    y = model(x)
    print(f"Evaluation mode output shape: {y.shape}")
